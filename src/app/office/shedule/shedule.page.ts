...
async addOnShedule(day: any, empl: User){
  if(this.user.employee.access < 3){
    return
  } else {
  const options = [
    '7:00 - 17:00',
  ...
  if(choise){
    ...
    let end = 0
    let hours = 0
    if(choise === '7:00 - 17:00'){
      start = dayDate.setHours(7, 0, 0, 0)
      end = dayDate.setHours(17, 0, 0, 0)
    ...
    if(choise === '8:00 - 17:00'){
      start = dayDate.setHours(8, 0, 0, 0)
      end = dayDate.setHours(17, 0, 0, 0)
      hours = 1 + (end-start) / 1000 / 60 / 60
    }
    if(choise === '10:00 - 17:00'){
      start = dayDate.setHours(10, 0, 0, 0)
      end = dayDate.setHours(17, 0, 0, 0)
      hours = 1 + (end-start) / 1000 / 60 / 60
    }
    if(choise === 'Custom'){
