...
import { NirService } from 'src/app/office/CRUD/nir/nir.service';
...
export class AddEntryPage implements OnInit {
  ...
  async startEntryFlow(){
    ...
    if(date){
      ...
      if(tip){
        ...
        showToast(this.toastCtrl, 'Trebuie să alegi tipul de cheltuială', 2000, 'error-toast')
        this.modalCtrl.dismiss(null)
      }
    }
  }
  ...
  confirm(){
    if(this.form.valid && this.date){
      ...
      this.http.post(`${this.url}register/add-entry`, entry).subscribe(response => {
